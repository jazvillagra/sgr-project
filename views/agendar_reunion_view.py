from tkinter import Frame, Label, Button, END, StringVar, Radiobutton, IntVar
...
class AgendarReunionView:
  ...
  def agregar_datos_callback(self, ventana, frame, detalle, organizador, rol_organizador, cant_participantes, fecha_realizacion):
    ...
    Label(frame, text="Elegir sala:", width=20).grid(row=3, column=2)
    row_count = 10
    while not sala_reunion:
      ...
      while cont < 0:
        ...
        for i in salas_disponibles:
          cont += 1
          var = StringVar()
          var.set(Radiobutton(frame, text=i.nombre_sala, variable=var, value=i.nombre_sala).grid(row=row_count, column=3))
          row_count += 1
        if (cont < 0):
      ...
      sgte_btn = Button(frame, text="Siguiente", activeforeground="black", width=20, command=lambda: self.sel_tipo_reu_callback(ventana, frame, detalle, organizador, rol_organizador, cant_participantes, sala_reunion, estado_reunion, fecha_realizacion, hora_inicio, hora_finalizacion))
  ...
  def agendar_callback(self, frame, detalle, organizador, rol_organizador, cant_participantes, sala_reunion, estado_reunion, fecha_realizacion, hora_inicio, hora_finalizacion, tipo):
    ...
    if tipo == 1:
      ...
      fecha_finalizacion = anho.get() + "-" + mes.get() + "-" + dia.get()

      agendar_reunion_btn = Button(frame, text="Agendar", activeforeground="black", width=20, command=lambda: self.agendar_reunion_periodica_callback(detalle, organizador, rol_organizador, cant_participantes, sala_reunion, estado_reunion, fecha_realizacion, fecha_registro, hora_inicio, hora_finalizacion, fecha_finalizacion, int(frecuencia.get())))
